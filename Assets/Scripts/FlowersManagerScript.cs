...
public class FlowersManagerScript : MonoBehaviour
{
    ...
    private Image[] FlowersImages;
    private TextMeshProUGUI[] FlowersNumbers;
    private RectTransform scrollTransform;

    private void Start()
    {
        FlowersImages = new Image[numberOfFlowers];
        FlowersNumbers = new TextMeshProUGUI[numberOfFlowers];
        ...
        scrollTransform.anchoredPosition = new Vector2((float)(Index - (FlowersObjects.Length - 1) / 2.0)*100, 0);
        ...
        {
            var xLocation = (float)(i - (FlowersObjects.Length - 1) / 2.0)*100;
    ...
    public void PlaceFlower(Vector2 position)
    {
        ...
        {
            FlowersCount[Index]--;
            if(FlowersCount[Index] >= 0)FlowersNumbers[Index].text = FlowersCount[Index].ToString();
